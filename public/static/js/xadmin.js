;!function (win) {
  ...
  Xadmin.prototype.end = function() {
    ...
    Xadmin.prototype.refresh = function () {
        var index = xadmin.tabsPage.index;
        if (index >= $('.x-iframe').length) {
            index = $('.x-iframe').length -1
        }
        $('.x-iframe').eq(index).attr("src",$('.x-iframe').eq(index).attr('src'));
    }
...
layui.use(['layer','element','jquery'],function() {
    ...
    element.on('tabDelete(xbs_tab)', function(data){
        ...
        xadmin.del_data(id);
        if (data.index < xadmin.tabsPage.index){
            xadmin.tabsPage.index--
        }
    });
    element.on('tab(xbs_tab)', function(data){
        xadmin.tabsPage.index = data.index
    });
    //左侧菜单
    ...
    // 隐藏左侧
    $('.container .layui-nav .left_open i').click(function(event) {
        if($('.left-nav').css('width')=='220px'){
            $('.left-nav .open').click();
